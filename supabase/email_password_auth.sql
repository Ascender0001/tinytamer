...
create unique index if not exists idx_player_saves_user_id_unique on public.player_saves(user_id);
  ...
  if current_user in ('anon', 'authenticated') and auth.uid() is not null then
    ...
    if tg_op = 'UPDATE' then
      if old.id is distinct from new.id then
        raise exception 'Users cannot change profile id';
      end if;

      if old.email is distinct from new.email then
        raise exception 'Users cannot change profile email';
      end if;
      ...
      if old.created_at is distinct from new.created_at then
        raise exception 'Users cannot change profile creation time';
      end if;
...
create trigger protect_profile_approval_and_private_fields
before insert or update on public.profiles
for each row execute function public.protect_profile_approval_and_private_fields();
...
drop policy if exists "No public approved users access" on public.approved_users;
create policy "No public approved users access"
